# eslint-plugin-fold — implementation handoff
...
So fill exactly that gap. `fold/breaks` inserts and removes line breaks and does
nothing else. It has no opinion on semicolons, quotes, or spacing, so it conflicts
with nothing, so there is no `eslint-config-fold` and never will be. You add a
rule; you don't disable anything.
...
It converges because §5 exists. Pass 1 it may insert a break that turns out to be
unnecessary once semicolons and quotes settle. Pass 2 it sees the settled source
and the removal pass takes that break back out. Bidirectionality was already in
the design for Prettier parity; convergence is the same property viewed from the
fix loop.
...
Ordered by how often they matter. Ship the first six; the rest are stretch.
...
| 4 | Binary / logical operators | **before** the operator |
...
| 14 | Template literal `${}` interiors | last resort |
...
**Never delete a newline that is part of a blank line.** The removal pass (§5)
deletes exactly one newline plus its attached indentation. If joining two
fragments would require consuming a run of newlines, don't — that's a vertical
spacing change wearing a collapse costume. Concretely:

```js
// Fold does not touch this, even though the call would fit on one line
foo(
  a,
...
This is the same rule as §5's blank-line freeze, viewed from the edit level
rather than the group level. A blank line anywhere inside a group is a signal the
author grouped something deliberately, and Fold has no way to know what, so it
leaves the group alone entirely.

**Consequence for measuring.** Blank lines are zero-width and never trigger the
addition pass. When computing whether a collapsed group fits, count only the text
that would actually be joined.
...
Break before the operator, at one precedence level at a time, descending only if
...
function expression, or arrow function, and it is the only such argument in the
list.
...
For parity with Prettier, Fold is bidirectional. Breaks that aren't earning
their keep get collapsed.

**Collapse when:** the joined result fits within `maxWidth` *and* the break is
not protected.

**Protected from collapse:**

- All necessary breaks (§3.1)
- Chains containing block bodies (§4.4)
- Any group containing a comment
- Any group whose interior contains a blank line — a blank line is a strong
  signal of deliberate semantic grouping, so the whole group is frozen as-is
...
**Decided, not configurable:** a group with no blank line and no comment that the
author broke by hand *is* collapsed when it fits. Prettier does the same (modulo
its object-literal first-line heuristic). Preserving would be friendlier;
collapsing is more consistent, and consistency is the pitch. There is no
`respectExistingBreaks` flag — see §7. Authors who need a layout protected use a
...
4. Removal pass: for each existing break, is it protected? If not, would
   the joined result fit? If yes → emit a delete edit.
5. Measure lines against the post-removal projection.
...
Steps 4 and 6 interact: a removal can make a line too long and an addition can
make removals possible. Compute the *projection* rather than applying and
re-measuring naively.
...
all-or-nothing lists, the chain exception, hugging, collapse behavior — is fixed.
...
This closes §5's open question by construction: with no `respectExistingBreaks`
escape hatch, collapse behavior is decided outright, and it's decided as
*collapse*. §3.4's blank-line freeze already gives authors a precise, zero-config
way to protect a layout.
...
only — so **decide it: a tab counts as 4 columns.**

Fold's indent inference (§7) already tells it whether the file uses tabs; this is
the separate question of how wide to *score* one. 4 matches `max-len`'s default,
so a project running both won't see them disagree about which lines are too long.
Document it in the README; don't make it configurable.
...
These are the tests that matter. Write them before the clever parts.
...
4. **Width compliance.** Every output line ≤ `maxWidth`, except lines with an
   empty break-candidate set (§7.1). Assert the exception is *computed*, not
   tolerated: a line over width must either have been broken or have no legal
   break position. A line that had a candidate and wasn't broken is a bug.
5. **Differential corpus.** Run over a few large real repos alongside Prettier
   and diff the diffs. Not to match Prettier — to find cases where Fold's
   output is *worse*, and to build the doc's "how we differ" section.

---

## 9. Build order

1. Plugin skeleton: `fold/breaks` reporting over-width lines with accurate
   locations and no fixes at all. Wire up `RuleTester`. Confirm it runs under a
   TS + JSX config without touching a parser ourselves — syntax parity is proven
   before any formatting logic exists.
2. `format()` as a pure function returning an empty edit list; rule adapter emits
   whatever it returns. The seam exists from day one so the harness in step 4 can
   bypass ESLint.
3. Break-candidate tree for optional-break categories 1–3 (calls, arrays,
   objects), addition only, outermost-first.
4. AST-equivalence and idempotence harness over a real corpus, against `format()`
   directly. Do not defer this — idempotence is what makes the rule architecture
   legal (§2.2), so it needs to fail loudly from early on.
5. Binary operators by precedence (category 4), forbidden-break table.
6. Method chains + the block-body exception (§4.4). Signature behavior; own test
   file.
7. Removal pass (§5). Convergence under the real `--fix` loop becomes testable
   here — add test 3.
8. Hugging (§4.5).
9. Remaining optional-break categories, in table order.
10. Docs, `recommended` config, README positioning per §1.
